...
jQuery(document).ready(function ($) {
    ...
    $(".fma_field").each(function () {
        var current = $(this),
            ...
            autocomplete_config = {
                ...
                noCache: formidable_autocomplete_field.config[fieldId].autocomplete_cache === "true",
                showNoSuggestionNotice: formidable_autocomplete_field.config[fieldId].autocomplete_validate === "true",
                ...
                params: {
                    ...
                    '_ajax_nonce': formidable_autocomplete_field.ajaxnonce,
                    'target_form': target_form,
                    'target_field': target_field,
                    'target_field_type': target_field_type,
                    'target_field_data_target': target_field_data_target
                },
                ...
                success: function (newRow) {
                    ...
                    jQuery.ajax({
                        ...
                        success: function (newOptions) {
                            ...
                            $("[name='item_meta[" + triggerFieldArgs.dependents_id[newOptionParese.index] + "]']").change();
                        }
                    });
    ...
    $("span.clear-button").on("click", function (event) {
        var item_number = $(this).siblings(".fma_field").attr('name').substr(10, 3);
        clearChild(item_number);
        $(this).siblings(".fma_field").autocomplete().clear();
        $(this).siblings(".fma_field").val('');
        $(this).fadeOut();
    });

    function clearChild(parentItemNumber) {
        var elements = $("[field_filter=" + parentItemNumber + "]");
