...
class GFireMAutocompleteAdmin {
	...
	private function get_result( $field_id, $search, $target_field_type, $field_filter = false, $group = false, $start_field = false, $parent_field = false, $limit = - 1 ) {
		global $wpdb;
		$cache_name    = 'meta_value_for_' . $parent_field . '_' . $field_filter . '_' . $field_id . '_' . $search;
		$cached_result = get_transient( $cache_name );
		$suggestions   = ( $cached_result !== false ) ? $cached_result : array();
		$arguments     = array();
	...
	public function get_autocomplete_line() {
		...
		if ( isset( $_GET["field_id"] ) && isset( $_GET["target_form"] ) && isset( $_GET["autocomplete_values"] ) && isset( $_GET["index"] ) ) {
			...
			$cached_result       = get_transient( $cache_name );
			$result              = ( $cached_result !== false ) ? $cached_result : new stdClass();
			if ( $cached_result === false ) {
				$arguments       = array();
